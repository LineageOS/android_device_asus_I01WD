...
const std::string kBreath = "breath";
const std::string kBrightness = "brightness";
const std::string kDelayOff = "delay_off";
const std::string kDelayOn = "delay_on";
...
static void set(const std::string& path, const std::string& value) {
    ...
    if (!file.is_open()) {
        LOG(DEBUG) << "failed to write " << value.c_str()
                   << "to " << path.c_str();
        return;
    }
